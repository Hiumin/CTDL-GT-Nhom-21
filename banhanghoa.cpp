...
void tinhLuongtheoThang(PNodeNV P, int thangcc , int namcc){
    ...
    long double luongThang = 0;
    int phutlamthemThang = 0;
    long double tienthuongThang = 0;
    int phutphatThang = 0;
    long double tienphatThang = 0;
    NoveNVLuong* R = P->info.Luong;
    bool tim = false;
    while(R != NULL){
        if(R->data.chamcong.thangcc == thangcc && R->data.chamcong.namcc == namcc){
            R->data.tienphat = (R->data.chamcong.phutphat/60.0) * P->info.luongcoban;
            R->data.tienthuong = (R->data.chamcong.phutlamthem/60.0) * P->info.luongcoban;
            long double luongNgay = P->info.luongcoban*((GIORA - GIOVAO) + (PHUTRA - PHUTVA0)/60.0) - R->data.tienphat + R->data.tienthuong;
...
void tinhLuongtheoNam(PNodeNV P, int namcc){
    ...
    long double luongNam = 0;
    int phutlamthemNam = 0;
    long double tienthuongNam = 0;
    int phutphatNam = 0;
    long double tienphatNam = 0;
    NoveNVLuong* R = P->info.Luong;
    bool tim = false;
    while(R != NULL){
    ...
    while (P != NULL) {
        ...
        while (R != NULL) {
            if (R->data.chamcong.thangcc == thang && R->data.chamcong.namcc == nam) {
                long double tienPhat = (R->data.chamcong.phutphat / 60.0) * P->info.luongcoban;
                long double tienThuong = (R->data.chamcong.phutlamthem / 60.0) * P->info.luongcoban;
                long double luongNgay = P->info.luongcoban * ((GIORA - GIOVAO) + (PHUTRA - PHUTVA0) / 60.0) - tienPhat + tienThuong;
                ...
                K.chamcong.namcc = timeinfo->tm_year + 1900; // Năm
                    ...
                    case 'A':{
                        ...
                        if(K.chamcong.phutphat <= 0){
                            ...
                            cout << "Ban da di muuon " << K.chamcong.phutphat << " phut." << endl;
                        }
                        break;
                    }
                    case 'b': 
